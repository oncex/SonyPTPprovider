...
using com::sony::imaging::remote::LiveViewImage;
using com::sony::imaging::remote::SDIDevicePropInfoDataset;
using com::sony::imaging::remote::SDIDevicePropInfoDatasetArray;
...
int Command::_send(com::sony::imaging::remote::socc_ptp *ptp,
  ...
  int ret;
  if (PTPTransaction::DATA_IS_STRING == t->size) {
    // string
    ...
    *(uint8_t *)_data = string_length;
    ...
    ALOGD("PTP command:  send > code=0x%04X, n=%d, p1=0x%08X, p2=0x%08X, p3=0x%08X, p4=0x%08X, "
    ...
    ALOGD("PTP command:  send < ret=%d, session=%d, transaction=%d, code=0x%04X, n=%d, "
    ...
    delete[] send_data;
  } else if (PTPTransaction::DATA_IS_FILE == t->size) {
    int err = 0;
    ...
    do {
      ...
      if (0 == err) {
        ALOGD("PTP command:  send > code=0x%04X, n=%d, p1=0x%08X, p2=0x%08X, p3=0x%08X, "
            "p4=0x%08X, p5=0x%08X, file=\"%s\", size=%lld\n",
            t->code, t->nparam, t->params[0], t->params[1], t->params[2],
            t->params[3], t->params[4], t->data.file,
    ...
    switch (err) {
      case 1:
        ALOGD("PTP command:  don't send anything. cannot get the status of %s\n", t->data.file);
        break;
      case 2:
        ALOGD("PTP command:  don't send anything. cannot open the file(%s). the reason is %s\n",
            t->data.file, strerror(errno));
        break;
      case 3:
        ALOGD("PTP command:  don't send anything. cannot malloc the memory. the size is %lld\n",
            static_cast<long long>(file_stat.st_size));
        break;
      case 4:
        ALOGD("PTP command:  don't send anything. cannot read the file(%s). the reason is %s\n",
            t->data.file, strerror(errno));
        break;
    }
  } else {
...
int Command::_recv(com::sony::imaging::remote::socc_ptp *ptp,
  ...
  com::sony::imaging::remote::Container res;
  int ret;
  ALOGD("PTP command:  recv > code=0x%04X, n=%d, p1=0x%08X, p2=0x%08X, p3=0x%08X, p4=0x%08X, "
      "p5=0x%08X\n",
      t->code, t->nparam, t->params[0], t->params[1], t->params[2],
  ...
  ALOGD("PTP command:  recv < ret=%d, session=%d, transaction=%d, code=0x%04X, n=%d, "
...
int Command::wait(com::sony::imaging::remote::socc_ptp *ptp) {
  int ret;
  com::sony::imaging::remote::Container res;
  ALOGD("PTP command:  wait >\n");
  ret = ptp->wait_event(res);
  ALOGD("PTP command:  wait < ret=%d, session=%d, transaction=%d, code=0x%04X, n=%d, "
      "p1=0x%08X, p2=0x%08X, p3=0x%08X, p4=0x%08X, p5=0x%08X\n",
      ret, res.session_id, res.transaction_id, res.code, res.nparam, res.param1,
...
int Command::reset(com::sony::imaging::remote::socc_ptp *ptp) {
  int ret;
  ALOGD("PTP command:  reset >\n");
  ret = ptp->reset();
  ALOGD("PTP command:  reset < ret=%d\n", ret);
  return ret;
}

int Command::clear_halt(com::sony::imaging::remote::socc_ptp *ptp) {
  int ret;
  ALOGD("PTP command:  clear_halt >\n");
  ret = ptp->clear_halt();
  ALOGD("PTP command:  clear_halt < ret=%d\n", ret);
  return ret;
}
...
int Command::get(com::sony::imaging::remote::socc_ptp *ptp,
  ...
  if (NULL != data) {
    ...
    ALOGD("PTP command:  cannot find the data of 0x%04X\n", device_property_code);
  }
  delete info;
